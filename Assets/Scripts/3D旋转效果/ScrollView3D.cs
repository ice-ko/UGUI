...
public class ScrollView3D : MonoBehaviour
{
    ...
    // 控制“深度”的曲线（在3d版本只是Z值，在2D UI中你可以使用深度（NGUI））
    // 注意：
    // 1.在ugui中，窗口小部件的深度可能会导致性能问题
    // 2.如果您使用3D UI，只需设置项目的Z位置
    [Tooltip(" 控制“深度”的曲线")]
    ...
    private ScrollViewItem curCenterItem;
    private ScrollViewItem preCenterItem;
    ...
    private ScrollViewDragController dragController;
    ...
    public List<ScrollViewItem> listEnhanceItems;
    // sort to get right index
    private List<ScrollViewItem> listSortedItems = new List<ScrollViewItem>();

    private static ScrollView3D instance;
    public static ScrollView3D GetInstance
    {
        get { return instance; }
    }
    ...
    void Start()
    {
        ...
        for (int i = count - 1; i >= 0; i--)
        {
            ...
            DragView script = obj.GetComponent<DragView>();
            if (script != null)
        ...
        // sorted items
        listSortedItems = new List<ScrollViewItem>(listEnhanceItems.ToArray());
    ...
    public void UpdateEnhanceScrollView(float fValue)
    {
        ...
        {
            ScrollViewItem itemScript = listEnhanceItems[i];
    ...
    private int GetMoveCurveFactorCount(ScrollViewItem preCenterItem, ScrollViewItem newCenterItem)
    {
        SortEnhanceItem();
    ...
    static public int SortPosition(ScrollViewItem a, ScrollViewItem b) { return a.transform.localPosition.x.CompareTo(b.transform.localPosition.x); }
    ...
    public void SetHorizontalTargetItemIndex(ScrollViewItem selectItem)
    {
        if (!canChangeItem)
            return;
